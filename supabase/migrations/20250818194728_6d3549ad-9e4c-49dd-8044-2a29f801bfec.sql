...
CREATE POLICY "Allow public read access for eod-screenshots" 
ON storage.objects 
FOR SELECT 
USING (bucket_id = 'eod-screenshots');

-- Allow authenticated users to upload their own EOD screenshots
CREATE POLICY "Allow users to upload their own EOD screenshots" 
ON storage.objects 
FOR INSERT 
WITH CHECK (
...
CREATE POLICY "Allow users to update their own EOD screenshots" 
ON storage.objects 
FOR UPDATE 
USING (
  bucket_id = 'eod-screenshots' 
...
CREATE POLICY "Allow users to delete their own EOD screenshots" 
ON storage.objects 
FOR DELETE 
USING (
  bucket_id = 'eod-screenshots' 
