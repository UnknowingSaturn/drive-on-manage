...
serve(async (req) => {
  ...
  try {
    ...
    if (userExists) {
      ...
      // Update/ensure profile exists for existing user
      ...
      if (upsertError) {
        ...
        // Don't throw error - this is not critical for driver creation
      }
    } else {
    ...
    // Send invitation email for all cases
    ...
    if (!userExists) {
      ...
      emailSubject = "Welcome to the Driver Portal - Account Created";
      emailContent = `
      ...
      // Existing user - notify about driver role
      emailSubject = "You've been added as a Driver - Access Your Account";
      emailContent = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <h2 style="color: #333; text-align: center;">You're now a Driver!</h2>
          
          <p>Hi ${firstName},</p>
          
          <p>You've been added as a driver to our team. You can now access the driver portal using your existing account credentials.</p>
          ...
          <ol>
            <li>Click the login button below</li>
            <li>Use your existing password to sign in</li>
    ...
    return new Response(JSON.stringify({
      success: true,
      userId: userId,
      message: userExists ? 'Driver profile created for existing user' : 'Driver invited successfully',
